...
(function () {
    ...
    var VERSION = "1.0.0",
        ...
        __config = {
            ...
            postUrl: "http://139.224.1.36:3001/log"     //异常信息提交的服务器地址
            // postUrl: "http://localhost:3001/log"     //异常信息提交的服务器地址
        };
    ...
    var getPostData = function (level, msg) {

        var body = {};
        if (__config.postContextInfo) {
            body = {
                secret: 'bN525JGfxoQ561o3K58Ng6E6vnY3ROd9',
        ...
        if (msg instanceof Error) {
            ...
            body = extend(body, {logBody: JSON.stringify({type: 'error', level: 'error', message: msg.message, stack: msg.stack})});
        }
        else {
            // body = extend(body, {name: "", message: msg + "", stack: ""});
            body = extend(body, {logBody: JSON.stringify({type: 'event', level: level, message: msg + ""})});
        }

        return body;
    }
    var post = function (level, msg) {
        var body = getPostData(level, msg);
        //依赖于jquery\zepto
        try {
